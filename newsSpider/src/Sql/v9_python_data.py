...
class v9_python_data(object):
    def __init__(self):
        self.__table = mySqlHelper()
    
    #查询单行
    def selectOneFromV9_data(self,newid):
        sql = 'select * from v9_python_data where id=%s'
        prames = (newid,)
        return self.__table.selectOne(sql, prames)
    
    #全部查询
    def selectAllFromV9_data(self):
        sql = 'select * from v9_python_data'
        return self.__table.selectall(sql)
    
    #插入单行
       
    def insertDataToV9_data(self,id,content,paginationtype,maxcharperpage,template,paytype):
        sql = 'insert into v9_python_data(id,content,paginationtype,maxcharperpage,template,paytype) values(%s,%s,%s,%s,%s,%s)'
        prames = (id,content,paginationtype,maxcharperpage,template,paytype,)
    ...
    def deleteFromV9_data(self,news_id):
        sql = 'delete * from v9_python_data where id=%d'
        prames = (news_id,)
        return self.__table.delete(sql, prames)
    #删除全部
    def deleteAllFromV9_data(self):
        sql = 'delete from v9_python_data'
