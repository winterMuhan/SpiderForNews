...
class v9_python(object):
    ...
    def selectFromV9For15(self,id):
        sql = 'select * from v9_python where id limit %s,15'
        prames = (id,)
    ...
    def selectUrlFromV9(self):
        sql = 'select url from v9_python'
        return self.__table.selectall(sql)
    
    #插入单行数据
       
    def insertDataToV9(self,id,catid,typeid,title,keywords,description,posids,url,listorder,status,sysadd,islink,username,inputtime,updatetime):
        sql = 'insert into v9_python(id,catid,typeid,title,keywords,description,posids,url,listorder,status,sysadd,islink,username,inputtime,updatetime) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
        prames = (id,catid,typeid,title,keywords,description,posids,url,listorder,status,sysadd,islink,username,inputtime,updatetime,)
